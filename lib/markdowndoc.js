...
module.exports = {
  init: (app) => {
    // const markdowndoc = app.services.MarkdowndocService.markdowndoc
  },
  ...
  addRoutes: app => {
    ...
    let sitemap
    try {
      sitemap = sitemapper.init(docs, prefix)
    }
    catch (err) {
      console.log('ERROR in sitemapper',err)
      throw err
    }
    ...
    let routesMaker
    try {
      routesMaker = routesmaker.routesMaker
    }
    catch (err) {
      console.log('ERROR in routesMaker',err)
      throw err
    }
    ...
    routes = _.map(routes, (inDocRoute, index) => {
      ...
      if (fuzzyMatch != -1) {
        ...
        const route = app.config.routes[fuzzyMatch]
